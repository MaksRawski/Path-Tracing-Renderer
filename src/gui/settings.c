...
static bool GuiSettings_Scene(AppState *state) {
  ...
  if (igButton("Load scene", button_size)) {
    ...
    if (GuiFileBrowser_open(chosen_path.str, sizeof(SmallString))) {
      state->settings.scene_paths.new_scene_path = chosen_path;
      changed = true;
    }
  }
  ...
  if (igCombo_Str_arr("BVH type", (int *)&state->settings.BVH_build_strat,
    ...
    state->settings.BVH_build_strat_changed = true;
  }

  if (!SmallString_is_empty(&state->settings.scene_paths.loaded_scene_path)) {
...
static bool GUISettings_Camera(AppState *state) {
  bool changed = false;
...
static bool GuiSettings_Rendering(AppState *state) {
  ...
  changed |= igSliderFloat("Diverge strength",
                           &state->settings.rendering_params.diverge_strength,
...
void GuiSettings_draw(GUIOverlay *gui, AppState *state) {
  ...
  if (igCollapsingHeader_TreeNodeFlags("Scene",
    ...
    state->settings.scene_paths_changed |= GuiSettings_Scene(state);
  }
  if (igCollapsingHeader_TreeNodeFlags("Camera", 0)) {
    state->settings.cam_changed |= GUISettings_Camera(state);
  }
  if (igCollapsingHeader_TreeNodeFlags("Rendering", 0)) {
    state->settings.rendering_params_changed |= GuiSettings_Rendering(state);
  }
