...
void AppState_display(AppState *app_state, Renderer *renderer, GUIOverlay *gui,
  ...
  if (!Scene_is_empty(&app_state->scene)) {
    ...
    if (infinite_progressive_rendering || should_render_new_frame) {
      ...
      // NOTE: getting to this point doesn't mean that the frame (or even the
      // one before this one) has finished rendering, as we're simply queuing
      // commands for rendering. Those times seem to only have meaning when
      // rendering at least 3 frames (on my machine).
