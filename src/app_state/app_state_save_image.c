...
void AppState_save_image(AppStateSaveImageInfo *save_image_info, GLuint fbo,
  ...
  if (save_image_info->to_save) {
    ...
    void *pixels =
        malloc(resolution.width * resolution.height * BYTES_PER_PIXEL);
    ...
    if (stbi_write_png(save_image_info->path.str, resolution.width,
                       resolution.height, 3,
                       pixels, stride)) {
      printf("Sucessfully saved image to %s\n", save_image_info->path.str);
    }
