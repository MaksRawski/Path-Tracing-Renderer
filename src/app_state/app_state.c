...
void AppState_update_scene(AppState *app_state, Renderer *renderer) {
  ...
  if (app_state->settings.scene_paths_changed &&
      FilePath_exists(new_scene_path->str)) {
    app_state->settings.scene_paths_changed = false;
...
void AppState_update_camera(AppState *app_state, Renderer *renderer,
  ...
  if (app_state->settings.cam_changed) {
    app_state->settings.cam_changed = false;
    AppState__set_camera(app_state, renderer);
  } else if (app_state->settings.movement_enabled &&
             app_state->_renderer_focused &&
             Inputs_update_camera(
                 &app_state->settings.cam, events,
...
void AppState_update_renderer_parameters(AppState *app_state,
                                         Renderer *renderer) {
  if (app_state->settings.rendering_params_changed) {
    app_state->settings.rendering_params_changed = false;
...
void AppState_post_rendering(AppState *app_state, Renderer *renderer) {
  ...
  bool should_save_image =
      (app_state->settings.save_after_rendering && rendering_finished) ||
